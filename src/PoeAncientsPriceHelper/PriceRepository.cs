...
internal sealed class PriceRepository : IDisposable
{
    ...
    private async Task<bool> FetchAndMergeAsync(AppConfig config, CancellationToken ct)
    {
        try
        {
            ...
            // Bound concurrency even over HTTP/2: 23 simultaneous streams are needlessly bursty and
            // can trigger community-API throttling. Four keeps startup quick without being abusive.
            using var gate = new SemaphoreSlim(4);
            async Task<Dictionary<string, PriceEntry>> Limited(Func<Task<Dictionary<string, PriceEntry>>> fetch)
            {
                await gate.WaitAsync(ct);
                try { return await fetch(); }
                finally { gate.Release(); }
            }
            var exchangeTasks = ExchangeTypes.Select(type =>
                Limited(() => FetchTypeAsync(config.LeagueName, type, ct)));
            var itemTasks = ItemTypes.Select(type =>
                Limited(() => FetchItemTypeAsync(config.LeagueName, type, ct)));
            var results = await Task.WhenAll(exchangeTasks.Concat(itemTasks));
    ...
    private async Task<Dictionary<string, PriceEntry>> FetchTypeAsync(string league, string type, CancellationToken ct)
    {
        ...
        req.Headers.TryAddWithoutValidation("User-Agent", "Poe2GroundLootPriceHelper/1.3.1 (independent fork)");
    ...
    private async Task<Dictionary<string, PriceEntry>> FetchItemTypeAsync(string league, string type, CancellationToken ct)
    {
        var slug = league.Replace(" ", "").ToLowerInvariant();
        var typeSlug = type.ToLowerInvariant();
        ...
        return ParseItemResponse(await resp.Content.ReadAsStringAsync(ct));
    }

    internal static Dictionary<string, PriceEntry> ParseItemResponse(string json)
    {
        ...
        try
        {
            ...
            decimal divinePerPrimary = primary == "divine" ? 1m : rates?["divine"]?.Value<decimal>() ?? 0m;
            decimal exaltedPerPrimary = primary == "exalted" ? 1m : rates?["exalted"]?.Value<decimal>() ?? 1m;
            if (obj["lines"] is not JArray lines) return result;
            foreach (var line in lines)
            {
                var name = line["name"]?.Value<string>();
                var value = line["primaryValue"]?.Value<decimal?>();
                var key = NameNormalizer.Normalize(name ?? "");
                if (key.Length == 0) continue;
                var entry = value is null
                    ? new PriceEntry(0m, 0m, HasMarketData: false)
                    : new PriceEntry(value.Value * divinePerPrimary,
                        Math.Round(value.Value * exaltedPerPrimary, 1));
                if (!result.TryGetValue(key, out var old) ||
                    (!old.HasMarketData || (entry.HasMarketData && entry.DivineValue < old.DivineValue)))
                    result[key] = entry;
            }
        }
        catch (Exception ex) { Console.Error.WriteLine($"[PriceRepository] item parse failed: {ex.Message}"); }
        return result;
    }
