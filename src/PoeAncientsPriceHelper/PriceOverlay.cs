...
internal sealed record PriceRow(int CenterY, string OcrText, decimal DivineValue, decimal ExaltedValue, bool HasPrice, int Multiplier = 1, string Name = "", bool ExactMatch = false, MemeKind Meme = MemeKind.None, bool MultiplierExplicit = false, int PriceX = 0, Rectangle LabelBounds = default);
...
internal sealed class PriceOverlayForm : Form
{
    ...
    private void PaintScene(Graphics g)
    {
        ...
        int defaultPriceX = _regionRect.Right + _xOffset;
        ...
        foreach (var row in _rows)
        {
            // Ground-loot rows already carry absolute OCR bounds. Use their actual on-screen centre
            // directly instead of rebuilding Y from the overlay's region: a window/client-origin
            // mismatch (notably windowed PoE with a title-bar/non-client inset) otherwise adds a
            // constant offset and puts every price underneath its label. Legacy panel rows have no
            // LabelBounds and keep the original region-relative path.
            int screenY = ScreenYForRow(row, _regionRect);

            // Debug layer: per-row boxes + the OCR text for rows that didn't resolve to a price.
            if (_debug)
            {
                var rowBox = !row.LabelBounds.IsEmpty
                    ? row.LabelBounds
                    : new Rectangle(ocrLeft, screenY - RowHalfHeight, ocrRight - ocrLeft, RowHalfHeight * 2);
                if (row.HasPrice)
                {
                ...
                else
                {
                    ...
                    g.DrawString($"? {row.OcrText}", _debugFont, grayBrush, defaultPriceX, screenY - 7);
                }
            }
            ...
            if (row.HasPrice)
            {
                ...
                int priceX = row.PriceX > 0 ? row.PriceX : defaultPriceX;
                DrawPrice(g, row, priceX, screenY, isTop);
            }
        }
    }

    internal static int ScreenYForRow(PriceRow row, Rectangle regionRect) =>
        !row.LabelBounds.IsEmpty
            ? row.LabelBounds.Top + row.LabelBounds.Height / 2
            : regionRect.Top + row.CenterY;
