# PoE 2 Ground Loot + Detailed Trade Price Helper — fork 1.2.0
...
Version 1.2.0 improves the integrated equipment check with a no-focus passive overlay, clearer item/filter layout, and a lowest-listings table. Hover an item in PoE 2 and press the game's normal `Ctrl+C`; the automatic search no longer activates the helper window. Choose Edit filters only when you need keyboard input, preferably while PoE 2 uses Borderless mode.
...
Run `install/Poe2GroundLootPriceHelper-v1.2.0-Setup.exe`. It installs per user, requires no administrator rights, and includes the required .NET runtime. Close an older running copy before installing.
...
See the [developer documentation](docs/README.md), [overlay module notes](overlay/README.md), [fork provenance](FORK_NOTES.md), [changelog](CHANGELOG.md), and [1.2.0 release record](docs/releases/1.2.0.md).
